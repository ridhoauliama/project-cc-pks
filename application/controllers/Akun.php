...
class Akun extends CI_Controller
{
	...
	{
		echo $this->input->post('username', true) . '<br>';
		echo $this->input->post('pl', true) . '<br>';
		echo $this->input->post('pb1', true) . '<br>';
		echo $this->input->post('pb2', true) . '<br>';

		if (empty($this->input->post('username', true))) {
			redirect(base_url('akun/pengaturan_akun'));
		}

		$x = $this->db->get_where('t_user', ['username', $this->input->post('username', true)])->row_array();
		$pw = $x['password'];
		...
		if ($pl == $this->input->post('pb1', true)) {
			$this->session->set_flashdata('akun_alert', '
			<div class="alert alert-danger p-1 text-center">
				<small>Password Lama dan Password Baru sama.</small>
			</div>');
			redirect(base_url('akun/pengaturan_akun'));
		}
	}
